...
block_options = { ...
    struct('num_blocks', 1, 'batch_size', 1, ...
        'StepTolerance', 1e-6), ...
    struct('num_blocks', 2, 'batch_size', 2, ...
        'StepTolerance', [1e-6; 1e-4]), ...
    struct('num_blocks', 2, 'batch_size', 1, ...
        'grouped_direction_indices', {{[1, 3], 2}}, ...
        'StepTolerance', [1e-5; 1e-3])};

for i = 1:numel(block_options)
    options = block_options{i};
    ...
    production = run_solver(@bds, x0, disable_acceleration(options));
    reference = run_solver( ...
        @bds_without_acceleration_reference, x0, options);
    assert(isequaln(production, reference), ...
        'Frozen BDS reference differs for automatic block configuration %d.', i);
end
