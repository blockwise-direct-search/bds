...
fprintf('GRADIENT_STOPPING_THRESHOLD_OK\n');
    ...
    function verify_reference_regimes()
        ...
        for reference_norm = reference_norms
            ...
            for i = 1:numel(current_norms)
                [bds_result, bds_calls] = run_bds_case( ...
                    reference_norm, current_norms(i), 1, 5, grad_tol);
                [accelerated_result, accelerated_calls] = run_accelerated_case( ...
                    reference_norm, current_norms(i), 1, 5, grad_tol, false);

                assert_same_result(bds_result, accelerated_result);
                assert(isequal(bds_calls, accelerated_calls), ...
                    'Acceleration-off changed the objective-call sequence.');
                assert(bds_result.exitflag == expected_exitflags(i), ...
                    'Unexpected decision at a gradient threshold boundary.');
                if expected_exitflags(i) == 5
                    assert(bds_result.output.funcCount == 5, ...
    ...
    function verify_window_requires_every_entry()
        ...
        assert(bds_result.exitflag == 5 && bds_result.output.funcCount == 11, ...
    ...
    function verify_acceleration_on_matches_lean()
        ...
        assert(isequal(accelerated_calls, lean_calls), ...
            'Acceleration-on and lean used different objective-call sequences.');
        assert(isequaln(accelerated_result.x, lean_result.x) ...
                && isequaln(accelerated_result.f, lean_result.f) ...
                && accelerated_result.exitflag == lean_result.exitflag ...
                && accelerated_result.output.funcCount == lean_result.output.funcCount, ...
            'Acceleration-on differs from lean at the reference-scaled threshold.');
        assert(accelerated_result.exitflag == 5 ...
                && accelerated_result.output.funcCount == 5, ...
    ...
    function verify_removed_option_is_rejected()
        ...
        assert_throws(@() bds(@(x) x^2, 0, removed_options), ...
            'BDS:set_options:UnknownField');
        assert_throws(@() accelerated_bds_options( ...
            @(x) x^2, 0, removed_options), ...
            'accelerated_bds_options:RemovedGradientReferenceRelativeTolerance');
        assert_throws(@() lean_evolved_bds(@(x) x^2, 0, removed_options), ...
            'lean_evolved_bds:RemovedGradientReferenceRelativeTolerance');
    end

    function [result, calls] = run_bds_case( ...
            reference_norm, later_norms, window_size, maxfun, grad_tol)
        options = stopping_options(window_size, maxfun, grad_tol);
        [result, calls] = run_solver(@bds, options, reference_norm, later_norms);
    end

    function [result, calls] = run_accelerated_case( ...
        ...
        solver = @(fun, x0, solver_options) ...
            accelerated_bds_options(fun, x0, solver_options);
        [result, calls] = run_solver(solver, options, reference_norm, later_norms);
    end
...
assert(isequaln(first.x, second.x) ...
    ...
    'Acceleration-off differs from bds at a gradient threshold boundary.');
