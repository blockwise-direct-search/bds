# Deferred review of the `accelerated_bds_options` main function

This note records possible cleanup in the main body of
`accelerated_bds_options.m` that is intentionally deferred until the solver's
...
that exit flag. This behavior is inherited from `src/bds.m`; it must not be
changed only in `accelerated_bds_options.m`, because doing so would break the
acceleration-off equivalence contract.
...
1. With all acceleration switches disabled,
   `accelerated_bds_options.m` matches `src/bds.m`.
2. With all acceleration switches enabled,
   `accelerated_bds_options.m` matches `lean_evolved_bds.m` through both the
